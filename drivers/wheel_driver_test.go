...
func (s *WheelDriverTestSuite) TestWheelDriver() {
	...
	s.T().Run("test toggle", func(t *testing.T) {
		...
		err := s.driver.Toggle()
		assert.NoError(err)
		assert.Equal(test.CheckValue{Pin: "1", Val: 1}, <-s.checkChan)
		assert.Equal(test.CheckValue{Pin: "2", Val: 0}, <-s.checkChan)
	})
	s.T().Run("test stop", func(t *testing.T) {
		assert := require.New(t)
		err := s.driver.Stop()
		assert.NoError(err)
		assert.Equal(test.CheckValue{Pin: "1", Val: 0}, <-s.checkChan)
		assert.Equal(test.CheckValue{Pin: "2", Val: 0}, <-s.checkChan)
	})
	...
	s.T().Run("test back", func(t *testing.T) {
		...
		assert.NoError(err)
		assert.Equal(test.CheckValue{Pin: "1", Val: 0}, <-s.checkChan)
		assert.Equal(test.CheckValue{Pin: "2", Val: 1}, <-s.checkChan)
	})
}
